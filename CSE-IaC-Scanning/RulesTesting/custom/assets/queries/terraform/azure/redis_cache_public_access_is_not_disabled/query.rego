...
CxPolicy[result] {
	...
	not common_lib.valid_key(azresource, "public_network_access_enabled")
	...
	result := {
		...
		"keyExpectedValue": sprintf("azurerm_redis_cache[%s].public_network_access_enabled should be defined and set to false", [name]),
		"keyActualValue": sprintf("azurerm_redis_cache[%s].public_network_access_enabled is undefined", [name]),
