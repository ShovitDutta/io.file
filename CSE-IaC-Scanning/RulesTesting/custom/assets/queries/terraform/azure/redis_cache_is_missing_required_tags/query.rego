...
CxPolicy[result] {
	azresource := input.document[i].resource.azurerm_redis_cache[name]
	not azresource.tags.costcenter
	result := {
		"documentId": input.document[i].id,
		"resourceType": "azurerm_redis_cache",
		"resourceName": tf_lib.get_resource_name(azresource, name),
...
CxPolicy[result] {
	azresource := input.document[i].resource.azurerm_redis_cache[name]
	not azresource.tags.dataclassification
...
CxPolicy[result] {
	azresource := input.document[i].resource.azurerm_redis_cache[name]
	not azresource.tags.environmenttype
...
CxPolicy[result] {
	azresource := input.document[i].resource.azurerm_redis_cache[name]
	not azresource.tags.sharedemailaddress
