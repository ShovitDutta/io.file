...
process_azure_cloud_tfp_file ()
{
    ...
    ECHO "INFO: JQ-1 .configuration.provider_config"
    ...
    if [ "${tfp_get_key}" == "tfp_rc" ]; then
                ...
                ECHO "INFO: JQ-3.tfp_rc_alsp"
    fi
    ECHO "INFO: JQ-3 .resource_changes and tfp_rc_tbsp tfp_rc_alsp "
    #get_prior_state. prior_state (ps=plan_Prior_State)
    tfp_ps=$(echo "tfp_ps")
    tfp_ps_azure_subscription_id=${tfp_ps_azure_subscription_id:-NONE}
    ...
    if [ "${tfp_get_key}" == "tfp_ps" ]; then
                ...
                #tfp_ps_azure_subscription_id=$(cat ${1} | jq '.prior_state.values.root_module.child_modules[].resources[0].values.subscription_id?' | sed 's/\"//g' |  sed 's/null//')
                tfp_ps_azure_subscription_id=$(cat ${1} | jq '.prior_state?|.values?|select( . != null)|.root_module.child_modules[].resources[0].values.subscription_id?' | sed 's/\"//g' |  sed 's/null//')
                tfp_ps_azure_subscription_id=${tfp_ps_azure_subscription_id:-NONE}
    fi
    ECHO "INFO: JQ-4 .prior_state"
    ...
    tfp_pc_azure_subscription_id=${tfp_pc_azure_subscription_id:-NONE}
    let tfp_pc_rcp_ct=0
    let tfp_pc_rcm_ct=0
    let tfp_pc_ars_ct=0
    ...
    if [ "${tfp_get_key}" == "tfp_pc" ]; then
                ...
                if [ "${tfp_pc_ars_ct}" -gt  0 ]; then
                            tfp_pc_azure_subscription_id=$(cat ${1} | jq '.configuration.provider_config.azurerm.expressions.subscription_id.constant_value?' | sed 's/\"//g' |  sed 's/null//')
                            tfp_pc_azure_subscription_id=${tfp_pc_azure_subscription_id:-NONE}
                fi
    fi
    ECHO "INFO: JQ-5 .configuration.provider_config"
    azure_subscription_id=$(echo "0000-0000-0000-0000-0000")
    if [ "${tfp_ps_azure_subscription_id}" == "NONE" ]; then
       if [ "${tfp_pc_azure_subscription_id}" == "NONE" ]; then
              azure_subscription_id=$(echo "0000-0000-0000-0000-0000")
       else
              azure_subscription_id=$(echo "${tfp_pc_azure_subscription_id}")
       fi
    else
              azure_subscription_id=$(echo "${tfp_ps_azure_subscription_id}")
    fi
...
#---------------------
         ECHO "INFO: File ${1} has Planned_Values, Any Rules Violated or NON-COMPLIANCE_ACTION: SCAN"
                      ...
                      tfp_pv_srp=$(echo "${tfp_pv}" | jq '.[]|[ { "service_reource":.type,"service_name":.name, "resources_from": .resources_from, "properties_already_set":.values, "tags_already_set":.values.tags}]' | jq -s '.')
...
#---------------------
    ...
    if [ "${get_tag_for_this_tfp_from_pv}" != "tfp_pv_tag" ]; then
          echo "${get_tag_for_this_tfp_from_pv}" | sed 's/applicationname/appid/' >  ${3}/${2}_${4}_tfp_pv_tag.json
          tfp_pv_tag_info_hash[${2}_${4}_tfp_pv_tag_key]=$(echo "${get_tag_for_this_tfp_from_pv}" | jq '.|keys|join("|")' | sed 's/applicationname/appid/')
          ...
          tfp_pv_tag_info_hash[${2}_${4}_tfp_pv_tag_json]=$(cat ${3}/${2}_${4}_tfp_pv_tag.json | sed 's/applicationname/appid/')
    ...
    if [ "${get_tag_for_this_tfp_from_rc_tbsp}" != "tfp_rc_tbsp_tag" ]; then
          echo "${get_tag_for_this_tfp_from_rc_tbsp}" | sed 's/applicationname/appid/'  > ${3}/${2}_${4}_tfp_rc_tbsp_tag.json
          tfp_rc_tag_info_hash[${2}_${4}_tfp_rc_tbsp_tag_key]=$(echo "${get_tag_for_this_tfp_from_rc_tbsp}" | jq '.|keys|join("|")' | sed 's/applicationname/appid/')
          ...
          tfp_rc_tag_info_hash[${2}_${4}_tfp_rc_tbsp_tag_json]=$(cat ${3}/${2}_${4}_tfp_rc_tbsp_tag.json | sed 's/applicationname/appid/')
    ...
    if [ "${get_tag_for_this_tfp_from_rc_alsp}" != "tfp_rc_alsp_tag" ]; then
          echo "${get_tag_for_this_tfp_from_rc_alsp}" |  sed 's/applicationname/appid/' >  ${3}/${2}_${4}_tfp_rc_alsp_tag.json
          tfp_rc_tag_info_hash[${2}_${4}_tfp_rc_alsp_tag_key]=$(echo "${get_tag_for_this_tfp_from_rc_alsp}" | jq '.|keys|join("|")'  | sed 's/applicationname/appid/')
          ...
          tfp_rc_tag_info_hash[${2}_${4}_tfp_rc_alsp_tag_json]=$(cat ${3}/${2}_${4}_tfp_rc_alsp_tag.json  | sed 's/applicationname/appid/')
    ...
    if [ "${get_tag_for_this_tfp_from_ps}" != "tfp_ps_tag" ]; then
          echo "${get_tag_for_this_tfp_from_ps}" | sed 's/applicationname/appid/' > ${3}/${2}_${4}_tfp_ps_tag.json
          tfp_ps_tag_info_hash[${2}_${4}_tfp_ps_tag_key]=$(echo "${get_tag_for_this_tfp_from_ps}" | jq '.|keys|join("|")' | sed 's/applicationname/appid/')
          ...
          tfp_ps_tag_info_hash[${2}_${4}_tfp_ps_tag_json]=$(cat ${3}/${2}_${4}_tfp_ps_tag.json | sed 's/applicationname/appid/')
