  ...
  let my_process_num=8
      ...
      if [[ "${p1}" == "NONE" ]]  &&  [[ "${p2}" == "NONE" ]]; then
          ...
          if [ ${#rpt_daily_csv_ar[@]} -lt 3 ]; then
             ...
             get_run_date_from_file=$(echo "${daily_csv_ar[0]}" | cut -d "/" -f 6)
             ...
             do
               ...
               #/data/iac_scan_status/csv/2024/20240125-04-025/daily_csv/SCAN_R-20240125-21_29_42-1706218182-042024-025-1706218182-createSUB-CORP-EPACORE-hapdb-dev-centralus-ECS.csv 
               #cat ${all_scan_files} | cut -d "," -f 1,2,4,5,8,14,15,16,17
               date_key=(`echo "${all_scan_files}" | cut -d "/" -f 8 | cut -d "-" -f 2-6`)
               date_val=(`echo "${all_scan_files}" | cut -d "/" -f 8 | cut -d "-" -f 2-3 | sed 's/_/:/g'`)
             ...
             echo "vendor_rule_id,rule_name,pipeline_id,appname,timestamp,terraform_key,utc_time,result" > ${date_based_root_dir}/all_csv/SCAN_RESULT_ALL_${get_only_yyyymmdd}.csv

             db_writter_header=("vendor_rule_id","rule_name","pipeline_id","appname","timestamp","terraform_key","utc_time,result")
             
             final_json_ar=()
             for all in `cat ${scan_result_file} | grep -v "^pipeline_name" |  cut -d "," -f 1,2,3,4,5,6,8,9 | awk -F "," '{ OFS="^";  print $1,$2,$3,$5,$4,"|",$7"|"$6"|"$8;}'`
             do
                 #echo "INFO: ${all}"
                 F1=$(echo "${all}" | cut -d "|" -f 1)
                 F2=$(echo "${all}" | cut -d "|" -f 2)
                 F3=$(echo "${all}" | cut -d "|" -f 3)
                 F4=$(echo "${all}" | cut -d "|" -f 4)
                 #echo "INFO: ${F1},,,,, ${F2},     ${F3} ${F4}"
                 #col1=$(echo "${F1}" | tr '^' ',' | sed 's/\,$//'| awk -F "," '{ OFS=","; print $1,$2,$3,$4"."$5;}')
                 col1=$(echo "${F1}" | tr '^' ',' | sed 's/\,$//') #| awk -F "," '{ OFS=","; print $1,$2,$3,$4"."$5;}')

                      rules_id_ar=()
                      rules_desc_ar=()
                      rules_id_ar=(`echo "${F2}" | sed 's/\^//g'| tr '.' '\n'`)
                      rules_desc_ar=(`echo "${F3}" | sed 's/\^//g'| tr '.' '\n'`)
                      let i=0
                      rc=$(echo "PASS")
                      if [ ${F4} -gt 0 ]; then
                          rc=$(echo "FAIL")
                      ...
                      for all_ids in `echo "${rules_id_ar[@]}"`
                      do
                         echo "${all_ids},${rules_desc_ar[i]}" | sed 's/\^/\,/g' | sed 's/_/ /g' | awk -F ","  -v col1=${col1} -v result=${rc} '{ OFS=",";  print $2,$1,col1,result;}'
                         #final_json_ar=$(`echo "${all_ids},${rules_desc_ar[i]}" | sed 's/\^/\,/g' | sed 's/_/ /g' | awk -F ","  -v col1=${col1} -v result=${rc} '{ OFS="|";  print $2,$1,col1,result;}'`)
                         #echo "INFO: final_json_ar: ${final_json_ar}"
                         let i=i+1
                      done
             done  >> ${date_based_root_dir}/all_csv/SCAN_RESULT_ALL_${get_only_yyyymmdd}.csv
              ...
              for all_tfp_rules_combo in `cat ${scan_result_file} | grep -v "^pipeline_name" |  cut -d "," -f 4,5,6,8 | awk -F "," '{ print $2,$1,'\n',$3,$4;}' | sed 's/ /./' | sed 's/ /|/g' | sed 's/||/:/'`
               ...
               do    
                     k1=$(echo "${all_tfp_rules_combo}" | cut -d ":" -f 1); 
                     k2=$(echo "${all_tfp_rules_combo}" | cut -d ":" -f 2); 
                     k3=$(echo "${k2}" | cut -d "|" -f 1); 
                     k4=$(echo "${k2}" | cut -d "|" -f 2); 
                     echo "${k3}" | tr '.' '\n' > ${date_based_root_dir}/all_csv/${k1}.ruleid_rel_${get_only_yyyymmdd}.csv
                     echo "${k3}" | tr '.' '\n' | awk -v tfp_id=${k1} -v yyyymmdd=${get_only_yyyymmdd} -v ww=${WW} -v ddd=${NNN} '{ OFS=","; print tfp_id,$1,yyyymmdd,ww,ddd;}' | sed 's/\./\,/'  >> ${terraformid_ruleid_rel}
                     echo "${k4}" | tr '.' '\n' > ${date_based_root_dir}/all_csv/${k1}.property_rel_${get_only_yyyymmdd}.csv
                     echo "${k4}" | tr '.' '\n' | awk -v tfp_id=${k1} -v yyyymmdd=${get_only_yyyymmdd} -v ww=${WW} -v ddd=${NNN} '{ OFS=","; print tfp_id,$1,yyyymmdd,ww,ddd;}' | sed 's/\./\,/'  >>${terraformid_property_rel}

              #do
              #       tmp_ar=()
              #       tmp_ar=(`echo "${all_tfp_rules_combo}" | tr ':' '\n'`)
                     #S55121_656.1706826675:"4d080822-5ee2-49a4-8984-68f3d4c890fc"."38c71c00-c177-4cd7-8d36-cd1007cdb190"|"Key_Expiration_Not_Set"."Vault_Auditing_Disabled"
...
#S23595_670.1705976403:20240123-02_20_03-1705976403-042024-023|createSUB-CORP-APIM-NONPROD-apim-dev-centralus|ECS|S23595_670.1705976403
              ...
              echo "INFO: Loading final SCAN_RESULT_ALL_${get_only_yyyymmdd}.csv to GCP dev-cvs-scan-reports/iac_scan_processed"
              gcloud storage  cp ${date_based_root_dir}/all_csv/SCAN_RESULT_ALL_${get_only_yyyymmdd}.csv   gs://dev-cvs-scan-reports/iac_scan_processed/SCAN_RESULT_ALL_${get_only_yyyymmdd}.csv
          else
