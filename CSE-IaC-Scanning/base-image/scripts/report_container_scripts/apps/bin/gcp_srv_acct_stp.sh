...
#Cloud related OPS - Start
  ...
  my_current_buk_data=$(echo "current_buk_data")
...
test_remote_object_list ()
{
    ...
    echo "INFO: cmd-gcloud_list=${my_current_buk_data}"
    gcloud_list=$(gcloud storage ls gs://${my_current_buk_data} 2>/dev/null | wc -l)
...
process_proj_name ()
{
          ...
          my_current_buk_data=$(echo "${my_proj_json_info}" | jq -Mr '.current_buk_data') # | sed 's/\"//g')
