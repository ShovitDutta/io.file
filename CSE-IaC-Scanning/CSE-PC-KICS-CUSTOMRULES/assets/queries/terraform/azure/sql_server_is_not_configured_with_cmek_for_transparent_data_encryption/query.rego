...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("'azurerm_mssql_server[%s].transparent_data_encryption_key_vault_key_id' should be defined and not null", [name]),
		"keyActualValue": sprintf("'azurerm_mssql_server[%s].transparent_data_encryption_key_vault_key_id' is undefined or null", [name]),
...
CxPolicy[result] {
	resource := input.document[i].resource.azurerm_mssql_server[name]
	resource.transparent_data_encryption_key_vault_key_id == ""
	...
	result := {
		...
		"keyExpectedValue": sprintf("'azurerm_mssql_server[%s].transparent_data_encryption_key_vault_key_id' should be defined and not null", [name]),
		"keyActualValue": sprintf("'azurerm_mssql_server[%s].transparent_data_encryption_key_vault_key_id' is undefined or null", [name]),
		"searchLine": common_lib.build_search_line(["resource", "azurerm_mssql_server", name, "transparent_data_encryption_key_vault_key_id"], []),
		"remediation": "Set transparent_data_encryption_key_vault_key_id to a valid Key Vault key for Customer-managed Transparent Data Encryption",
		"remediationType": "addition",
	}
}
