...
CxPolicy[result] {
	door := input.document[i].resource.azurerm_frontdoor[name].frontend_endpoint[k]
	...
	result := {
		...
		"resourceName": tf_lib.get_resource_name(input.document[i].resource.azurerm_frontdoor[name], name),
