...
CxPolicy[result] {
	resource := input.document[i].resource.azurerm_network_watcher_flow_log[name].retention_policy[k]
	var := resource.days
	var < 90
	...
	result := {
		...
		"searchLine": common_lib.build_search_line(["resource", "azurerm_network_watcher_flow_log", name, "retention_policy"], []),
