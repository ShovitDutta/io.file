...
CxPolicy[result] {
	resource := input.document[i].resource.azurerm_role_definition[name].permissions[k]
	...
	result := {
		...
		"resourceName": tf_lib.get_resource_name(input.document[i].resource.azurerm_role_definition[name], name),
