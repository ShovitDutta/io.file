...
CxPolicy[result] {
	tdp := input.document[i].resource.azurerm_postgresql_server[name].threat_detection_policy[k]
	tdp.enabled == false

	result := {
		"documentId": input.document[i].id,
		"resourceType": "azurerm_postgresql_server",
		"resourceName": tf_lib.get_resource_name(input.document[i].resource.azurerm_postgresql_server[name], name),
