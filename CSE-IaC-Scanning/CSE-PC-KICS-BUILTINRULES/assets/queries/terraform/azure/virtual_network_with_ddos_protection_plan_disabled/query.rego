...
CxPolicy[result] {
	ddosplan := input.document[i].resource.azurerm_virtual_network[name].ddos_protection_plan[k]
	ddosplan.enable != true

	result := {
		"documentId": input.document[i].id,
		"resourceType": "azurerm_virtual_network",
		"resourceName": tf_lib.get_resource_name(input.document[i].resource.azurerm_virtual_network[name], name),
		...
		"searchLine": common_lib.build_search_line(["resource", "azurerm_virtual_network", name, "ddos_protection_plan"], []),
