...
CxPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("aws_default_security_group[%s]", [name, block]),
		...
		"searchLine": common_lib.build_search_line(["resource", "aws_default_security_group", name, block], []),
		"remediation": sprintf("Remove '%s' from aws_default_security_group[%s]", [block, name]),
...
CxPolicy[result] {
	...
	result := {
		...
		"searchLine": common_lib.build_search_line(["resource", "aws_default_security_group", name, block, cidrs[c]], []),
		"remediation": sprintf("Remove cidr value '%s' from aws_default_security_group[%s].%s", [acceptAll[a], name, block]),
		"remediationType": "removal",
	}
}
