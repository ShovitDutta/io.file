...
CxPolicy[result] {
    ...
    result := {
		...
		"keyExpectedValue": sprintf("%s[%s].policy should be misconfigured", [resourceType[idx],name]),
		"keyActualValue": sprintf("%s[%s].policy allows access to function (unqualified ARN) and its sub-resources, add another statement with \":*\" to function name", [resourceType[idx],name])
	}
}
