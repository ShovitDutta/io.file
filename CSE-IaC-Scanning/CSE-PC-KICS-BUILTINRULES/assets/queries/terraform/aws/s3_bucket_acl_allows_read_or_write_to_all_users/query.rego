...
CxPolicy[result] {
	...
	result := {
		...
		"keyActualValue": sprintf("aws_s3_bucket[%s].acl is equal '%s'", [name, resource.acl]),
...
CxPolicy[result] {
	...
	publicAccessACL(bucket_acl.acl)

	result := {
		"documentId": input.document[i].id,
		"resourceType": "aws_s3_bucket_acl",
		"resourceName": tf_lib.get_specific_resource_name(bucket_acl, "aws_s3_bucket_acl", name),
		"searchKey": sprintf("aws_s3_bucket_acl[%s].acl", [name]),
		"issueType": "IncorrectValue",
		"keyExpectedValue": sprintf("aws_s3_bucket_acl[%s].acl should be private", [name]),
		"keyActualValue": sprintf("aws_s3_bucket_acl[%s].acl is %s", [name, bucket_acl.acl]),
