...
CxPolicy[result] {
	dnssec_config := input.document[i].resource.google_dns_managed_zone[name].dnssec_config[k]	
	key_spec := dnssec_config.default_key_specs[j]
	...
	result := {
		...
		"remediation": "Use a different algorithm for DNSSEC key specification. For example, 'rsasha256' or refer to the documentation for more options.",
